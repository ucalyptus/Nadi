...
Feature: OpenRiver session runtime
  OpenRiver must preserve the core Aquifer invariants while running locally.

  Scenario: Echo command is durably recorded
    Given a local OpenRiver stack
  ...
  Scenario: Tool command bypasses the broker
    Given a local OpenRiver stack
    When I create a session for tenant "gherkin"
    And I send an uppercase tool command with text "openriver"
    Then the event log contains "tool.result" with text "OPENRIVER"
